...
async function getProjectsGitHub() {
	try {
		...
		const resposta = await fetch('https://api.github.com/users/memariaa/repos?sort=updated&per_page=5',)
        ...
        // Cria o Botão Deploy somente se o repositório tiver uma URL para deploy
        const botaoDeploy = repositorio.homepage ? `<a href="${repositorio.homepage}" target="_blank" class="botao">Deploy</a>` : ''

        const botoesAcao = `
            <div class="project-buttons">
            <a href="${repositorio.html_url}" target="_blank" class="botao">
                GitHub
            </a>
            ${botaoDeploy}
            </div>`
